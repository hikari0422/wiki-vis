...
export const SavedHistoryModal: React.FC<SavedHistoryModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md pointer-events-auto">
      ...
      <div className="relative w-full max-w-2xl max-h-[85vh] bg-white/90 backdrop-blur-2xl border border-slate-200/60 rounded-3xl shadow-2xl p-6 flex flex-col gap-4 animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
        
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <div className="flex flex-col">
            <h2 className="text-xl font-extrabold text-slate-800 tracking-tight flex items-center gap-2">
              <FolderOpen className="w-5.5 h-5.5 text-indigo-600" />
              我的歷史存檔紀錄
            </h2>
        ...
        <div className="flex-1 overflow-y-auto pr-1 flex flex-col gap-3 min-h-[300px]">
          {loading ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center text-rose-500 p-4 gap-2">
              <span className="text-sm font-bold">{error}</span>
              <button
                onClick={fetchGraphs}
                className="mt-2 text-xs font-bold bg-indigo-50 hover:bg-indigo-100 text-indigo-600 py-1.5 px-3 rounded-xl transition-all"
              >
                重新整理
              </button>
            </div>
          ) : graphs.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center text-slate-400 p-8 gap-3">
              <FolderOpen className="w-12 h-12 text-slate-300" />
              <div>
                <h3 className="text-sm font-bold text-slate-600">尚無雲端存檔</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 pb-2">
              {graphs.map((graph) => (
                <div
                  ...
                  className="group relative bg-slate-50/50 hover:bg-indigo-50/20 border border-slate-200/50 hover:border-indigo-200 rounded-2xl p-4 shadow-sm hover:shadow-md transition-all duration-300 cursor-pointer flex flex-col justify-between gap-3 min-h-[140px]"
                >
                  {/* Top info */}
                  <div className="min-w-0">
                    <div className="flex justify-between items-start gap-2">
                      ...
                      {graph.title && graph.title !== graph.rootTitle && (
                        <span className="shrink-0 text-[9px] font-bold px-1.5 py-0.5 bg-slate-200/60 text-slate-500 rounded-md">
                          {graph.rootTitle}
                        </span>
                      )}
                    </div>
                  ...
                  {/* Bottom Stats */}
                  <div className="flex items-center justify-between border-t border-slate-200/20 pt-2.5">
                    <div className="flex gap-3 text-slate-500">
                      ...
                      <div className="flex items-center gap-1 text-[10px] font-bold">
                        <span className="text-[10px] uppercase font-extrabold px-1 bg-indigo-50 text-indigo-600 rounded">
                    ...
                    <button
                      ...
                      className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-all active:scale-95 cursor-pointer disabled:opacity-40"
                      title="刪除此存檔"
                    >
                      {deletingId === graph.id ? (
        ...
        <div className="border-t border-slate-100 pt-3 flex justify-end">
          <button
            onClick={onClose}
            className="py-2 px-5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl text-xs font-bold transition-all active:scale-95 cursor-pointer"
          >
            關閉
          </button>
        </div>
