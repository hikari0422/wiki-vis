...
import { saveGraphToFirestore, recordSessionStats, type SavedGraph } from '../../services/firebase';
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      ...
      if (state.user && state.nodes.length > 0) {
        ...
        // 紀錄當前 session stats
        const maxDepth = Math.max(0, ...state.nodes.map(n => n.depth ?? 0));
        recordSessionStats(state.nodes.length, maxDepth).catch(console.error);
