...
interface UseGraphOperationsProps {
  ...
  setFocusSelectedTrigger: React.Dispatch<React.SetStateAction<number>>;
  isMobile: boolean;
...
export function useGraphOperations({
  ...
  const handleExplore = async (node: WikiNode) => {
    ...
    try {
      ...
      const candidateTitles = allTitles.filter((t) => !existingIds.has(t));

      const fetchedTitles = currentLimit <= 0
        ? candidateTitles
        : await filterValidWikiTitles(candidateTitles, node.lang, currentLimit, node.variant);

      // Build a helper to make a node object
      const makeNode = (title: string): WikiNode => ({
        id: title,
        label: title,
        loaded: false,
      ...
      setNodes((prevNodes) => {
        ...
        // First pass: validated titles
        for (const title of fetchedTitles) {
          if (currentLimit > 0 && newNodesToAdd.length >= currentLimit) break;
          if (!existingNodeIds.has(title)) {
            newNodesToAdd.push(makeNode(title));
            existingNodeIds.add(title);
          }
        }

        // Top-up: if still short, pull unvalidated from allTitles
        if (currentLimit > 0 && newNodesToAdd.length < currentLimit) {
          const usedSet = new Set(fetchedTitles.map(t => t.toLowerCase()));
          for (const title of allTitles) {
            if (newNodesToAdd.length >= currentLimit) break;
            if (!existingNodeIds.has(title) && !usedSet.has(title.toLowerCase())) {
              newNodesToAdd.push(makeNode(title));
              existingNodeIds.add(title);
            }
          }
        ...
        addedTitles = newNodesToAdd.map(n => n.id);
      ...
      setLinks((prevLinks) => {
        ...
        addedTitles.forEach((title) => {
          ...
          if (!existingLinkKeys.has(key)) {
            newLinksToAdd.push({ source: node.id, target: title });
  ...
  const handleNodeClick = useCallback((node: WikiNode) => {
    ...
    // Smoothly center on unexpanded nodes so the user can watch children appear around it
    if (!node.loaded && !node.isDeadEnd) {
      setFocusSelectedTrigger(prev => prev + 1);
    }

    if (!node.loaded && !node.loading && !node.isDeadEnd) {
      handleExplore(node);
    }
  }, [isMobile, addToHistory, setFocusSelectedTrigger]);
  ...
  const handleReSearch = async (node: WikiNode) => {
    ...
    try {
      ...
      setNodes((prevNodes) => {
        ...
        // Top-up from allTitles if still short
        if (currentLimit > 0 && newNodesToAdd.length < currentLimit) {
          const usedSet = new Set(fetchedTitles.map(t => t.toLowerCase()));
          for (const title of allTitles) {
            if (newNodesToAdd.length >= currentLimit) break;
